...
const en: Copy = {
  ...
  valueProps: {
    ...
    items: [
      ...
      },
      {
        ...
        body: "The app, local transcription, and local summaries are free. There are no servers behind mojiroku, so it costs $0 to operate — no subscription, no storage caps. If you want higher-quality cloud summaries, that's an opt-in you pay for directly with your own API key.",
      },
      {
  ...
  privacy: {
    ...
    body: "Recording, transcription, summarization — all of it happens inside this Mac. Transcripts and notes are generated by local AI models running on your own hardware. Your data lives only on your device, as SQLite and audio files under ~/Library/Application Support, and uninstalling removes it. Every local feature works in airplane mode. The network is used only when you yourself choose cloud summaries or calendar import. Meeting notes shouldn't be something you deposit with a vendor — they should be something you own.",
    bullets: [
      "Stored on-device by default. Recordings, transcripts, summaries, and models live only on your Mac — nothing reaches an external server unless you choose cloud summaries.",
      "Local features work in airplane mode. Networking happens only for cloud summaries (BYOK) and calendar import — both opt-in.",
  ...
  pricing: {
    ...
    body: "The app, transcription, and summaries all run on-device by default. Recordings and notes are stored in SQLite, on your Mac alone. There are no servers behind mojiroku, so it costs $0 to operate — which is exactly why it can be free. The cloud gets involved only when you choose BYOK summaries (Anthropic / OpenAI) or calendar import. Everything else works in airplane mode.",
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "Yes. Transcription, local summaries, speaker diarization, full-text search, meeting mode — all of it runs on-device. The network is used only when you choose cloud summaries (Claude / OpenAI) or import events from Google Calendar. Every other local feature works even in airplane mode.",
      },
      {
  ...
  footer: {
    ariaLabel: "Footer",
    tagline:
      "A meeting-notes app that records, transcribes, and summarizes entirely on-device. Free to use, $0 to run. The network is used only when you choose cloud summaries or calendar import.",
    links: [
      {
        title: "Download",
