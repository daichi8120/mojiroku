...
| **Phase 8** | 話者ライブラリ（端末内声紋識別） | 🚧 **実装完了①〜④（[ADR-0018](./decisions/ADR-0018_話者ライブラリの声紋照合.md) 方向性 go）**: ①声紋を `DiarizationResult` に露出（consolidation 既計算・再抽出なし）②store v4（speaker_embeddings/library/matches・cosine 1:N・leave-one-recording-out・最小エンロール尺ゲート）③Tauri コマンド＋3保存経路に声紋永続化④SpeakersView 実 CRUD＋詳細でサジェスト照合（τ 非自動確定＝サジェスト先行）。**コード・単体テスト緑**。基盤=スパイク実証（TitaNet 0.926 vs 別人 ≤0.61・新規 DL 不要）。⚠️ **実機での声紋永続/クロス照合の裏取りは未実施**・τ/最小尺は実運用較正。 |
