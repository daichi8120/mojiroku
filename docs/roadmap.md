...
### モック先行（プレビュー）画面 — ✅ 配布からは自動で外れる（2026-08-27, v0.5.3）

合意により、**まだバックエンド未実装の機能画面はモックデータで作り込む**（デザイン全体像を先に可視化する選択）。
これらは極小の「プレビュー」マーカーを付け、**実機能と区別**する。

当初の対象のうち、**会議モードと話者ライブラリは実機能になった**（Phase 7 / Phase 8・ADR-0018）。
連携もカレンダー・Notion・Slack は実機能で、モックとして残るのは会議プラットフォーム欄だけ。

**いまモックとして残っているのは 5 つ**（Issue #21 で棚卸し）: 横断ダイジェスト / 質問する（ローカル RAG）/
チャプター / 翻訳 / 会議プラットフォーム連携。前 2 つは Phase 9、次 2 つは Phase 10 に対応する。

> **配布ゲートへの申し送り（解消済み）**: 旧記述は「配布直前に切り替える判断を行う」だったが、
> **その切り替えが一度も実行されないまま v0.5.2 まで配布された**（Issue #21）。手順を人が覚えている前提にしたのが誤り。
> v0.5.3 で `MOCK_PREVIEW` を Vite の `DEV` に紐づけ、**開発サーバーでのみ見え、配布ビルドでは自動的に消える**ようにした。
> 以後この判断は不要。
...
- **🎯 品質ゲート**: 実在の複数話者・日本語会議音声で要約品質を実機評価。→ **実施済み: PASS**（Qwen2.5-7B Q4_K_M）。**2026-08-30 に取り直し**、16GB 以上の端末では Qwen3.5-9B へ差し替え（ADR-0030）。**2026-09-23 に 16GB 未満も Qwen3.5-4B へ差し替え**（簡体字を出力段階で止めたうえで、ADR-0043/0044）。
