...
fn main() {
    ...
    config
        .profile("Release")
        // mojiroku patch: whisper.cpp(ggml) の std::filesystem は macOS 10.15+ 専用。
        // Apple Silicon/Metal 前提なので deployment target を 11.0 に固定する。
        // cmake デフォルトの 10.13 だと release ビルドで "introduced in macOS 10.15"
        // 不可用エラーになる（ggml-backend-reg.cpp）。CXXFLAGS では cmake が後付けする
        // 10.13 に負けるため、ここで -D で確実に上書きする。
