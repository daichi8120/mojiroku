...
mod tests {
    ...
    fn jobs_cascade_on_recording_delete() {
        ...
        assert!(s
            .list_jobs(&["pending", "running", "done", "failed"])
            .unwrap()
            .is_empty());
    }
    ...
    fn old_jobs_default_to_turbo_and_new_jobs_keep_their_model() {
        ...
        assert_eq!(
            select_whisper_model(Some(&old.transcription_model)).file,
        ...
        store
            .enqueue_job("model-job", "model-test", "transcribe", &chosen)
            .unwrap();
